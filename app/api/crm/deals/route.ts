...
import { validateDealCreate } from "@/src/server/schemas";
...
export async function GET(request: NextRequest) {
  ...
  return json(await crmStore.listDeals(customerId, scope.workspaceIds));
}
...
export async function POST(request: NextRequest) {
  ...
  const result = validateDealCreate(await parseJson(request));
  ...
  const deal = await crmStore.createDeal(result.data);
